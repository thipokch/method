...
// typedef MtRootSheetPage<T> = MaterialExtendedPage<T>;
// typedef MtSheet<T> = CupertinoSheetPage<T>;

const BoxShadow _kDefaultBoxShadow =
    BoxShadow(blurRadius: 10, color: Colors.black12, spreadRadius: 5);

class MtRootSheetPage<T> extends Page<T> {
  const MtRootSheetPage({
    required this.child,
...
class _PageBasedMaterialWithModalsPageRoute<T>
  ...
  _PageBasedMaterialWithModalsPageRoute({
    required MtRootSheetPage<T> page,
  }) : super(settings: page, builder: (_) => page.child) {
    assert(opaque);
  }

  MtRootSheetPage<T> get _page => settings as MtRootSheetPage<T>;
...
class MtSheet<T> extends Page<T> {
  final Widget child;
