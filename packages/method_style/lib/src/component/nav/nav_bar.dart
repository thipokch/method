...
const Duration _kNavBarTitleFadeDuration = MtDurations.fast;
...
class MethodNavigationBar extends StatefulWidget
    implements cupertino.ObstructingPreferredSizeWidget {
  const MethodNavigationBar({
    Key? key,
    this.leading,
  ...
  @override
  State<MethodNavigationBar> createState() => _MethodNavigationBarState();
}

class _MethodNavigationBarState extends State<MethodNavigationBar> {
...
class MethodSliverNavigationBar extends StatefulWidget {
  ...
  @override
  State<MethodSliverNavigationBar> createState() =>
      _MethodSliverNavigationBarState();
}

class _MethodSliverNavigationBarState extends State<MethodSliverNavigationBar> {
  late _NavigationBarStaticComponentsKeys keys;
...
class _NavigationBarStaticComponents {
  ...
  static KeyedSubtree? createBackChevron({
    ...
    return KeyedSubtree(
      key: backChevronKey,
      child: const Icon(MtSymbols.chevronBack),
    );
  }
...
class MethodNavigationBarBackButton extends StatelessWidget {
  const MethodNavigationBarBackButton({
    Key? key,
    this.color,
    this.previousPageTitle,
    this.onPressed,
  ...
  Widget build(BuildContext context) {
    ...
    if (onPressed == null) {
      assert(
        ...
        'MethodNavigationBarBackButton should only be used in routes that can be popped',
      );
    }
    ...
    // return const Icon(MtSymbols.chevronBack);
...
class _NavigationBarComponentsTransition {
  ...
  Animation<double> fadeInFrom(double t, {Curve curve = MtEasing.easeIn}) {
    return animation.drive(fadeIn.chain(
      CurveTween(curve: Interval(t, 1.0, curve: curve)),
    ));
  }

  Animation<double> fadeOutBy(double t, {Curve curve = MtEasing.easeOut}) {
